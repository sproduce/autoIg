<div class="row row-table @if($event->deleted_at)deleteLine @endif" data-event="{{$event->eventId}}" data-id="{{$event->dataId}}">
            ...
            <div class="col-1" title="Дата постановления">
                {{$event->eventFullInfo->dateTimeOrder->format('d-m-Y')}}
            </div>            
            <div class="col-1" title="Оплатить до">
                {{$event->eventFullInfo->datePayMax->format('d-m-Y')}}
            </div>            
            <div class="col-1">
                {{$event->eventFullInfo->uin}}
            </div>            
