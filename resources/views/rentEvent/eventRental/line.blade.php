<div class="row row-table @if($event->deleted_at)deleteLine @endif" data-event="{{$event->eventId}}" data-id="{{$event->dataId}}">
            ...
            <div class="col-1">{{$event->eventObj->name}}</div>
            <div class="col-1 p-0">
            ...
            <div class="col-3">
               {{$event->dateTime->format("d-m-y H:i")}} <strong>-</strong> {{$event->dateTime->addMinute($event->duration-1)->format("d-m-y H:i")}} ({{$event->eventFullInfo->durationHours}})
            </div>
